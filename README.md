...
## 📊 Project Datasets (7 Core Files)
The system leverages 7 specialized datasets to provide accurate nutrition and risk analysis:
1. **Exercise_Recommendation_Dataset.xlsx**:Calorie & intensity mapping for exercise.
2. **Food_Safety_ML_Dataset.xlsx**: Risk assessment for food safety & contamination.
3. **nutribot_dataset.xlsx**: (New) User-specific dietary patterns & health data.
4. **Updated_Indian_Food_Nutrition_Dataset.xlsx**: (New) Comprehensive nutrition data for 1000+ Indian dishes.
5. **User_Daily_Intake.csv**: Processed record of user meal history.
6. **food_mapping.json**: AI category mapping for food recognition.
7. **feature_names.json**: ML model features for risk prediction.

## 🤖 AI Chat System
The Nutribot AI Chat is now **Context-Aware** and **Personality-Driven**:
- **Personalized Advice**: Analyzes your age, weight, diseases (Diabetes, Hypertension, etc.), and allergies in every reply.
- **Dynamic Nutrition Facts**: Scans food labels or searches database to compare against your daily calorie/sodium targets.
- **Disease-Specific Warnings**: Flags high GI for Diabetic users or high sodium for Hypertension.
- **Meal Suggestions**: Recommends Breakfast, Lunch, and Dinner based on your dietary preferences (Vegan, Vegetarian, Keto, etc.).
